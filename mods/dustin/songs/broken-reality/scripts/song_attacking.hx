...
function update(elapsed:Float) {
    if (!justPressedZ && (FlxG.keys.justPressed.SPACE || MusicBeatState.getState().hitbox.buttonExtra.justPressed)) justPressedZ = true;
    ...
    if (boyfriend.animation.name == "attack" && frameNum != boyfriend.animation.frameIndex && inAttack) {
        ...
        if (boyfriend.animation.frameIndex == 107) {
            if(Options.gameplayShaders) warp.distortion = 4;
            camGame.zoom += 0.35;
            if(Options.gameplayShaders) {
