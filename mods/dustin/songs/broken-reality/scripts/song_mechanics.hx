...
function goLeftScroll() {
    if (!FlxG.save.data.mechanics) return;
    desiredMultX = 1; desiredMultY = 0;
    for (i in 0...4) {
        (new FlxTimer()).start(i*.06, (_) -> {
            strumsOffsets[i][0] = (96 - strumLines.members[1].members[i].x) - 10;
            strumsOffsets[i][1] = (FlxG.height/2) + (-50 + ((104+10) * Math.abs(i-3))) - ((104+10)*2) + 50;

            FlxTween.cancelTweensOf(strumLines.members[1].members[i]);
            FlxTween.tween(strumLines.members[1].members[i], {angle: 0 - 360}, (Conductor.stepCrochet / 1000) * 4, {ease: FlxEase.circOut, onComplete: (_) -> {
                strumLines.members[1].members[i].angle = 0;
            }});
        });
    }

    desiredHudX = 283.5 + 120;
}
...
function goRightScroll() {
    ...
    for (i in 0...4) {
        (new FlxTimer()).start(i*.06, (_) -> {
            strumsOffsets[i][0] = (96 - strumLines.members[1].members[i].x) - 10 + FlxG.width - 300;
            strumsOffsets[i][1] = (FlxG.height/2) + (-50 + ((104+10) * Math.abs(i-3))) - ((104+10)*2) - 30;

            FlxTween.cancelTweensOf(strumLines.members[1].members[i]);
            FlxTween.tween(strumLines.members[1].members[i], {angle: -90 - 360}, (Conductor.stepCrochet / 1000) * 4, {ease: FlxEase.circOut, onComplete: (_) -> {
                strumLines.members[1].members[i].angle = -90;
            }});
        });
    }
