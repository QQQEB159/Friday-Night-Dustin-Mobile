...
function postCreate() {
    ...
    if(Options.gameplayShaders) {
        ...
        if(FlxG.save.data.blackwhite) for (cam in [FlxG.camera, camHUD, camCharacters]) cam.addShader(blackwhite);

        oldstatic = new CustomShader("static");
        oldstatic.time = 0; oldstatic.strength = 0;
        if(FlxG.save.data.static) for (cam in [FlxG.camera, camHUD, camCharacters]) cam.addShader(oldstatic);
...
function stepHit(step:Int) {
    switch (step) {
        ...
        case 1384:
            if(Options.gameplayShaders) {
            if (FlxG.save.data.distortion) camCharacters.addShader(chromWarp);
